import {
  ...
  const Postman = ({ rowData }) => {
    ...
    const [storageChangeFlag, setStorageChangeFlag] = useState(false); // Flag to track changes
    ...
    useEffect(() => {
      // localStorage se collectionData fetch karein
          ...
          if (collection.id === rowData.parentRowId) {
            collection.files.forEach((file) => {
              if (file.id === rowData.childRowId) {
                ...
                // console.log("file:--------------->>>", file.items);
                setLocalData(file.items[0]); // file.items ko set karenge state mein
              }
            });
          }
        });
      }
    }, [rowData, apiMethod]); // rowData jab change ho, toh effect chalega
  
    // console.log("localData:--------------->>>", localData.apiMethod);
