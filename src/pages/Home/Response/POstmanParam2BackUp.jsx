...
const PostmanParam2 = () => {
  const [queryString, setQueryString] = useState(""); // State to hold the query string
  ...
  const handleDelete = (index) => {
    ...
    if (postmanParams.length > 1) {
      ...
      setQueryString(`${baseURL}${queryParts.join("&")}`);
    }
  };
  ...
  return (
    <Stack sx={{ display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      <TextField
        label="Query String"
        value={queryString} // Bind the state value to the input
        fullWidth
        variant="outlined"
...
//     // Check if '?' is in the string, and handle query key-value addition
//     if (value.includes("?")) {
//       // Split the query string by "?" and get the part after it
//       const parts = value.split("?");
//       const queryPart = parts[1];
//       const keyValuePairs = queryPart.split("&");
...
//                     onChange={(e) => {
//                       const updatedParams = [...postmanParams];
//                       updatedParams[index].checked = e.target.checked;
//                       setPostmanParams(updatedParams);
...
// ==========================XXXXXXXXXXXXXXXXX  IInd attempt  XXXXXXXXXXXXXXXXXXXXXX=====================================================
...
//   // Function to update checkbox status when key or value is typed
//   const handleInputChange = (index, field, value) => {
//     const updatedParams = [...postmanParams];
//     updatedParams[index][field] = value;
...
//     const storedParams = JSON.parse(localStorage.getItem("postmanParams"));
//     if (storedParams && storedParams.length > 0) {
//       // Check if the last row is empty, if so, remove any extra empty rows
//       const lastRow = storedParams[storedParams.length - 1];
...
//       }
//     } else {
//       // Set the initial empty row if no data in localStorage
//       setPostmanParams([
//         {
//           id: generateUniqueId(),
//           key: "",
...
// ==========================XXXXXXXXXXXXXXXXX  Ist attempt  XXXXXXXXXXXXXXXXXXXXXX=====================================================
...
//         localStorage.setItem("postmanParams", JSON.stringify(newParams)); // Save to localStorage
//       } else {
//         // Clear parameters if no "?" operator
//         setPostmanParams([]);
//         localStorage.removeItem("postmanParams"); // Clear localStorage if no '?' is found
//       }
//     };
