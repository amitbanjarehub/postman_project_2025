...
const Params = ({ selectedRows, handleSelectedRowsChange }) => {
  // const [rows, setRows] = useState([
  //   { id: 1, key: "", value: "", description: "", checked: false },
  // ]);

  // Load rows from localStorage if available, else use default row
  const [rows, setRows] = useState(() => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("headerParamsRows", JSON.stringify(rows));

    if (JSON.stringify(selectedRows1) !== JSON.stringify(selectedRows)) {
      handleSelectedRowsChange(selectedRows1);
    }
  }, [rows, selectedRows1, selectedRows, handleSelectedRowsChange]);
